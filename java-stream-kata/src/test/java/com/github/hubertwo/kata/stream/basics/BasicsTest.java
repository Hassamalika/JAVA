...
class BasicsTest {
    ...
    void task1() {
        Stream<Fruit> sortedFruits = FRUITS.stream().sorted(Comparator.comparingInt(Fruit::getCalories).reversed()).map(f -> {
            System.out.println(f);
            return f;
        }).limit(2);
        List<Fruit> mostCaloricFruits = sortedFruits.collect(Collectors.toUnmodifiableList());// TODO:  FRUITS.stream()
    ...
    void task2() {
        final int sumOfCalories = FRUITS.stream().mapToInt(ob -> ob.getCalories() / 2).sum(); // TODO: FRUITS.stream()
    ...
    void task3() {
        final Map<Character, Set<Fruit>> mapOfFruits = Collections.emptyMap(FRUITS.stream().collect(); // TODO: FRUITS.stream()
